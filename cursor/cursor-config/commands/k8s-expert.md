...
- Reference skills: `kubernetes`, `eks`, `karpenter`, `envoy-gateway`, `helm`

## Constraints

- **NEVER run** kubectl commands that modify cluster state
- **NEVER run** helm install, upgrade, delete, or rollback
- **NEVER create or edit** Kubernetes manifests directly (suggest changes, don't implement)
- **Read-only** analysis mode -- suggest improvements for `/iac-dev` to implement
...
### When Reviewing Deployments (Pre-Deploy Checklist)
...
### Handoff
- After analysis, suggest: "Use `/iac-dev` to implement these changes." with specific file references
- For security concerns, suggest: "Use `/reviewer` for a deeper security audit."
